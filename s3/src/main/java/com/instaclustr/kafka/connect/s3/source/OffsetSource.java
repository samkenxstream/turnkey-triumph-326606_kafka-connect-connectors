...
public class OffsetSource {
	 ...
	 * 
	 * <Scenario> The consumer group is stopped and some of offset is deleted and
	 * pointing beginning offset for TopicPartition is higher then consumer group
	 * offset
	 * 
	 * consumerGroupOffset = CurrentOffset If consumerGroupOffset > totalRecords
	 * then totalRecords else consumerGroupOffset
	 * 
	 * <Scenario> Offsets are going to commit in batches.
	 */
	...
	public void syncGroupForOffset(TopicPartition topicPartition, Map<String, Long> consumerGroups, Long beginningOffset, Long totalRecords) {
		log.debug("topic partition : {}, BeginningOffset {} ", topicPartition, beginningOffset);
		for (Map.Entry<String, Long> entry : consumerGroups.entrySet()) {
			...
			Map<TopicPartition, OffsetAndMetadata> offsets = Collections.singletonMap(topicPartition,
					offsetAndMetadata);
